...
class BaseExecutionTimePredictorFallbackTest(unittest.TestCase):
    ...
    def _assert_fallback_before_assert(self, backend_section: str):
        fallback_check = "if tensor_parallel_communication_time == -1:"
        non_negative_assert = "assert tensor_parallel_communication_time >= 0"

        self.assertIn(fallback_check, backend_section)
        self.assertIn(non_negative_assert, backend_section)
        self.assertLess(
            backend_section.index(fallback_check),
            backend_section.index(non_negative_assert),
        )
    ...
    def test_simai_simulation_fallback_check_precedes_assert(self):
        ...
        self._assert_fallback_before_assert(backend_section)
